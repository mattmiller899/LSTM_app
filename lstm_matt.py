...
def get_args():
    ...
    parser.add_argument('-negname', dest='neg_name', help='name of neg file')
...
def load_data(pos_name, neg_name, NB_WORDS, MAX_LEN, k, s, in_dir):
    print('Loading seq data from {} and {}'.format(pos_name, neg_name))
    pos_seqs = [line[:-2] for line in open('%s/%s_%dgram_%dstride' % (in_dir, pos_name, k, s)) if len(line.split())>15]
    neg_seqs = [line[:-2] for line in open('%s/%s_%dgram_%dstride' % (in_dir, neg_name, k, s)) if len(line.split())>15]
...
def run_everything(gpu, k, s, batchsize, init, trainable, test, combined, total, iteration, in_dir, pos_name, neg_name, window):
    NB_WORDS = 20000
    MAX_LEN = 3000
    file_type = ""
    pos_name_train = '{}_train_{}.fasta'.format(pos_name, iteration)
    neg_name_train = '{}_train_{}.fasta'.format(neg_name, iteration)
    """ 
    vector_name = ''
    ...
    """
    X_train, y_train, kmer_index_train, n_seqs_train = load_data(pos_name_train, neg_name_train, NB_WORDS, MAX_LEN, k, s, in_dir)
    ...
    if init:
        ...
        kmer2vec={}
        if window is -1:
            f = open('%s/%s_%dgram_%dstride_vectors.txt' % (in_dir, pos_name_train, k, s))
        else:
            f = open('%s/%s_%dgram_%dstride_%dwindow_vectors.txt' % (in_dir, pos_name_train, k, s, window))
        for line in f:
            values = line.split()
            try:
                kmer = values[0]
